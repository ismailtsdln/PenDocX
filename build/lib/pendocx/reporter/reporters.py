...
class PDFReporter(BaseReporter):
    """Generates a PDF report from the HTML template."""
    
    def generate(self, mission: Mission, output_path: Path) -> Path:
        try:
            # We reuse the HTML generation logic
            html_reporter = HTMLReporter()
            # Temporary HTML file for conversion
            temp_html = output_path / "temp_report.html"
            html_reporter.generate(mission, output_path)
            
            # Since the HTMLReporter generates a file with a specific name, 
            # let's find it or just re-render the string.
            # Re-rendering is safer.
            from jinja2 import Template
            template = Template(html_reporter.__class__.generate.__globals__.get('template_str', "")) # This is hacky, let's just use the same template string
            
            # Better way: Move template string to a shared place or just use the same one here.
            # For simplicity in this chunk, I'll just re-define a simpler version or 
            # better, I'll make HTML template a class attribute later. 
            # For now, let's just implement the conversion.
            
            output_file = output_path / f"{mission.project_name}_report.pdf"
            
            html_file = output_path / f"{mission.project_name}_report.html"
            ...
            if pisa_status.err:
                raise ReporterError("Failed to generate PDF report: pisa error")

            logger.info(f"PDF report generated at [blue]{output_file}[/blue]")
            return output_file
        except Exception as e:
            raise ReporterError(f"Failed to generate PDF report: {e}")
